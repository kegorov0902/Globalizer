...
//                          All Rights Reserved.                           //
//                                                                         //
//  File:      noninjectiveevolvent.cpp                                    //
//                                                                         //
//  Purpose:   Source file for noninjective evolvent classes               //
//                                                                         //
...
\brief Реализация класса #NoninjectiveEvolvent

\details Реализация методов класса #NoninjectiveEvolvent
*/
...
namespace
{
        ...
        const Extended& extHalf)
    {
        ...
        double is_tmp = is.toDouble();
        double extZero_tmp = extZero.toDouble();
        double nexp_tmp = nexp.toDouble();
        double extOne_tmp = extOne.toDouble();
        ...
        const Extended& extHalf)
    {
        ...
        else if (key > 2)
        {
            ...
            //dr = dr - fmod(dr.toDouble(), extOne.toDouble()); // нужно заменить fmod на функцию для типа данных Extended
            dr=(dr>0)?floor(dr):ceil(dr, extZero);
            Extended dd = mne - dr;
            dr = d * dd;
            //dd = dr - fmod(dr.toDouble(), extOne.toDouble()); // нужно заменить fmod на функцию для типа данных Extended
            dd=(dr>0)?floor(dr):ceil(dr, extZero);
...
NoninjectiveEvolvent::NoninjectiveEvolvent(int _N, int _m, int _max_preimages) :
    Evolvent(_N, _m)
{
...
// ------------------------------------------------------------------------------------------------
NoninjectiveEvolvent::~NoninjectiveEvolvent()
{
}

// ------------------------------------------------------------------------------------------------
void NoninjectiveEvolvent::GetImage(const Extended& x, double* _y, int EvolventNum)
{
    if ((x < 0) || (x > 1))
    {
...
// ------------------------------------------------------------------------------------------------
int NoninjectiveEvolvent::GetNoninjectivePreimages(double* _y, Extended* x)
{
    ...
    if (N == 1)
        ...
        invmad(m, x, max_preimages, &preimNumber, y, N, 1, nexpExtended, mneExtended, extOne, extNull, extHalf);
    return preimNumber;
}
